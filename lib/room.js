...
function Room(jid, client) {
  logger.debug('Got room connect stanza,', client.jid.toString(), 'is joining', jid.toString())
...
Room.prototype.join = function() {
  ...
  this.room.on('error', function(err) {
    logger.log('disconnected from Talker because of ', err, 'reconnecting')
    self.room.destroy()
...
Room.prototype.handleMessage = function(stanza) {
  logger.debug('Received message stanza', stanza.getChild('body').text())
...
Room.prototype.handlePresence = function(stanza) {
  ...
  logger.debug('Received room presence stanza', stanza.toString())
...
Room.prototype._send = function(xml) {
  logger.debug('Sending stanza', xml.root().toString())
